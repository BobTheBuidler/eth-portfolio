...
PyInit_check(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("eth_portfolio__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "eth_portfolio__mypyc.ensure_deps");
    ...
    PyObject *capsule = PyObject_GetAttrString(tmp, "init_eth_portfolio_scripts___docker___check");
    Py_DECREF(tmp);
    if (capsule == NULL) return NULL;
    void *init_func = PyCapsule_GetPointer(capsule, "eth_portfolio__mypyc.init_eth_portfolio_scripts___docker___check");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit_check(); }
