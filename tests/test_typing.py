...
def test_remotetokenbalances_dataframe():
    rtb = RemoteTokenBalances({ 'protocol1': TokenBalances({ ADDRESS_1: Balance(balance=Decimal('10'), usd_value=Decimal('100')) }) })
    df = rtb.dataframe
    assert isinstance(df, DataFrame)
...
def test_walletbalances_dataframe():
    wb = WalletBalances({ 'assets': TokenBalances({ ADDRESS_1: Balance(balance=Decimal('10'), usd_value=Decimal('100')) }) })
    df = wb.dataframe
    assert isinstance(df, DataFrame)
...
def test_portfoliobalances_dataframe():
    pb = PortfolioBalances({ WALLET_1: WalletBalances({ 'assets': TokenBalances({ ADDRESS_1: Balance(balance=Decimal('10'), usd_value=Decimal('100')) }) }) })
    df = pb.dataframe
    assert isinstance(df, DataFrame)
